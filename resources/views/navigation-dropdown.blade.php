                ...
                <x-jet-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        ...
                        <div class="block px-4 py-2 text-xs text-gray-400">
                            {{ trans('profile.manage_account.title') }}
                        </div>
                        ...
                        @if (Laravel\Jetstream\Jetstream::hasApiFeatures())
                            <x-jet-dropdown-link href="/user/api-tokens">
                                {{ trans('profile.api_tokens.title') }}
                            </x-jet-dropdown-link>
                        @endif
                        ...
                        @if (Laravel\Jetstream\Jetstream::hasTeamFeatures())
                            <div class="block px-4 py-2 text-xs text-gray-400">
                                {{ trans('profile.manage_team.title') }}
                            </div>

                            <!-- Team Settings -->
                            <x-jet-dropdown-link href="/teams/{{ Auth::user()->currentTeam->id }}">
                                {{ trans('profile.manage_team.content.team_settings') }}
                            </x-jet-dropdown-link>

                            @can('create', Laravel\Jetstream\Jetstream::newTeamModel())
                                <x-jet-dropdown-link href="/teams/create">
                                    {{ trans('profile.manage_team.content.create_new_team') }}
                                </x-jet-dropdown-link>
                            @endcan
                            ...
                            <!-- Team Switcher -->
                            <div class="block px-4 py-2 text-xs text-gray-400">
                                {{ trans('profile.manage_team.content.switch_teams') }}
                            </div>
